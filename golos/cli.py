...
def legacyentry():
    ...
    parser.add_argument(
        ...
        version='%(prog)s {version}'.format(
            version=pkg_resources.require("golos")[0].version))
    ...
    parser_powerdown = subparsers.add_parser(
        'powerdown',
        help='Power down (start withdrawing STEEM from golos POWER)')
    ...
    if op[0] == "vote":
        ...
        if memos:
            ...
            if len(memo) > 0 and memo[0] == "#":
                ...
                # memo = golos.decode_memo(memo, op[1]["from"])
...
def steemtailentry():
    parser = argparse.ArgumentParser(
        description="UNIX tail(1)-like tool for the golos blockchain")
    parser.add_argument(
        '-f',
        '--follow',
